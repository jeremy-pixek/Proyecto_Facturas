...
const obtenerProductosFactura = async (facturaId) => {
    const query = 'SELECT * FROM transportedb.facturas.productosFactura WHERE factura_id = $1';
...
const obtenerTotalFactura = async (facturaId) => {
    const query = 'SELECT SUM(total) AS total FROM transportedb.facturas.productosFactura WHERE factura_id = $1';
    const result = await poolTransportedb.query(query, [facturaId]);
